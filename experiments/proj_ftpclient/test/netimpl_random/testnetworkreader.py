...
class RandTestNetworkReader(INetworkReader):
	def __init__(self, server, randServerConfig):
		self.readPos = 0
	...
	def readLine(self):
		...
		if self.config.READLINE_LEVEL == 0:
			return resp
		elif self.config.READLINE_LEVEL == 1:
	...
	def readAll(self):
		...
		if self.config.READALL_LEVEL == 0:
			return data
		elif self.config.READALL_LEVEL == 1:
	...
	def read(self):
		if self.readBuffer is None:
			...
			if self.config.READCHAR_LEVEL == 0:
				pass
			elif self.config.READCHAR_LEVEL == 1:
